...
STATIONS = {
    ...
    238: {'line': 14, 'name': 'Деловой центр'},
    239: {'line': 14, 'name': 'Шелепиха'},
    240: {'line': 14, 'name': 'Хорошёвская'},
    241: {'line': 14, 'name': 'ЦСКА'},
    242: {'line': 14, 'name': 'Петровский парк'},
...
NEW_LINES = {
    ...
    "8A": [
        ...
        -18+14, "Парк Победы",
        -18+11, "Минская",
        -18+7, "Ломоносовский проспект",
        -18+5, "Раменки",
        -18+0, "Мичуринский проспект",
        -18+6, "Озёрная",
        -18+9, "Говорово",
        -18+12, "Солнцево",
        -18+15, "Боровское шоссе",
        -18+17, "Новопеределкино",
        -18+19, "Рассказовка",
        -18+23, "Пыхтино",
        -18+27, "Аэропорт Внуково",
    ],
    2: [
        0, "Речной вокзал",
        3, "Беломорская",
    ...
    10: [
        ...
        12, "Верхние Лихоборы",
        14, "Окружная",
        16, "Петровско-Разумовская",
        19, "Фонвизинская",
        21, "Бутырская",
        25, "Марьина Роща",
        27, "Достоевская",
        30, "Трубная",
        32, "Сретенский бульвар",
        35, "Чкаловская",
        38, "Римская",
        41, "Крестьянская Застава",
        43, "Дубровка",
        45, "Кожуховская",
        49, "Печатники",
        52, "Волжская",
    ...
    11: [
        ...
        9, "Воронцовская",
        12, "Новаторская",
        14, "Проспект Вернадского",
        17, "Мичуринский проспект",
        19, "Аминьевская",
        22, "Давыдково",
        25, "Кунцевская",
        28, "Терехово",
        30, "Мнёвники",
        33, "Народное Ополчение",
        37, "Хорошёвская",
        39, "ЦСКА",
        42, "Петровский парк",
        34, "Савёловская",
        ...
        34+8, "Рижская",
        34+12, "Сокольники",
        34+15, "Электрозаводская",
        34+18, "Лефортово",
        34+22, "Авиамоторная",
        34+26, "Нижегородская",
        34+30, "Текстильщики",
        34+32, "Печатники",
        34+35, "Нагатинский Затон",
        34+38, "Кленовый бульвар",
        34+42, "Каширская",
        34+45, "Варшавская",
        34+45+5, "Каховская",
    ]
...
ADDITIONAL_CONNECTIONS = [
    (11, "Воронцовская", 6, "Калужская"),
    (11, "Новаторская", 16),
    (11, "Проспект Вернадского", 1),
    #(11, "Мичуринский проспект", "8А"),
    (11, "Кунцевская", 4),
    (11, "Кунцевская", 3),
    (4, "Кунцевская", 3),
    (11, "Хорошёвская", 7, "Полежаевская"),
    (11, "Петровский парк", 2, "Динамо"),
    (11, "Савёловская", 9),
    (11, "Марьина Роща", 10),
    (11, "Рижская", 6),
    (11, "Сокольники", 1),
    (11, "Электрозаводская", 3),
    (11, "Авиамоторная", 8),
    (11, "Нижегородская", 15),
    (11, "Нижегородская", 14),
    (14, "Нижегородская", 15),
    ...
    (11, "Печатники", 10),
    (11, "Каширская", 2),
    (11, "Каховская", 9, "Севастопольская"),
    (11, "Воронцовская", 6, "Калужская"),
    (16, "Новомосковская", 1),
    (10, "Зябликово", 2, "Красногвардейская"),
]

assert all(line and len(line) % 2 == 0 for line in NEW_LINES.values())
assert all(line and line[0] == 0 for line in NEW_LINES.values())
assert all(all(isinstance(x, str) for x in line[1::2]) for line in NEW_LINES.values())
assert all(all(isinstance(x, (int, float)) for x in line[0::2]) for line in NEW_LINES.values())

UNKNOWN_STATIONS = set((
    ("Ольховая", 1, ("Парк культуры", 1, 38)),
    ("Перерва", "D2", ()),
    ("Пыхтино", "8A", ("Киевская", 3, 43)),
...
def _onlyone(it):
    if len(L := list(it)) == 1:
        ...
        raise ValueError("Too much values" if L else "No values")

def _enrich_stations():
    from itertools import count
    counter_station = count(1+max(STATIONS))
    for line_number, line in NEW_LINES.items():
        prev_minutes = 0
        for d_cumul_minutes, x, y in zip(line[2::2], line[1::2], line[3::2]):
            station_x = {'line': line_number, 'name': x}
            station_y = {'line': line_number, 'name': y}
            
            def append_if_not_exists(station):
                if not list(STATIONS.values()).__contains__(station):
                    STATIONS[next(counter_station)] = station
                    
            def find_id(station):
                return _onlyone([x for x,y in STATIONS.items() if y == station])
            
            append_if_not_exists(station_x)
            append_if_not_exists(station_y)
            sid_x = find_id(station_x)
            sid_y = find_id(station_y)
            LINKS.append((sid_x, sid_y, 60 * (d_cumul_minutes - prev_minutes)))
            
            prev_minutes = d_cumul_minutes

def _enrich_stations2():
    from itertools import count
    counter_station = count(1+max(STATIONS))
    for args in ADDITIONAL_CONNECTIONS:
        match len(args):
            case 3:
                line_x, name, line_y = args
                name_x = name_y = name
            case 4:
                line_x, name_x, line_y, name_y = args
            case _:
                raise ValueError
        ...
        sid_x, sid_y = map(find_id, ({'name': name_x, 'line': line_x}, {'name': name_y, 'line': line_y}))
        LINKS.append((sid_x, sid_y, 2))
